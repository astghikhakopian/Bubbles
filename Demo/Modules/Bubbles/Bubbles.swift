...
@objc public protocol BubblesDelegate: class {
    func bubbles(_ bubbles: Bubbles, didSelect node: Node)
    func bubbles(_ bubbles: Bubbles, didDeselect node: Node)
    @objc optional func bubbles(_ bubbles: Bubbles, didRemove node: Node)
}

@objcMembers open class Bubbles: SKScene {
    
    /**
     The field node that accelerates the nodes.
     */
    open lazy var bubblesField: SKFieldNode = { [unowned self] in
     ...
     */
    open weak var bubblesDelegate: BubblesDelegate?
    ...
    func configure() {
        ...
        bubblesField.region = SKRegion(radius: radius)
        bubblesField.minimumRadius = radius
        bubblesField.strength = strength
        bubblesField.position = CGPoint(x: size.width / 2, y: size.height / 2)
    }
...
extension Bubbles {
    ...
    override open func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if removeNodeOnLongPress && !node.isSelected {
            ...
            if (timeDiff >= longPressDuration) {
                node.removedAnimation {
                    self.bubblesDelegate?.bubbles?(self, didRemove: node)
                }
                return
            }
        }
        ...
        if node.isSelected {
            ...
            bubblesDelegate?.bubbles(self, didDeselect: node)
        } else {
            if !allowsMultipleSelection, let selectedNode = selectedChildren.first {
                selectedNode.isSelected = false
                bubblesDelegate?.bubbles(self, didDeselect: selectedNode)
            }
            node.isSelected = true
            bubblesDelegate?.bubbles(self, didSelect: node)
        }
    }
...
extension Bubbles {
    ...
    func sortedNodes() -> [Node] {
        return children.compactMap { $0 as? Node }.sorted { node, nextNode in
            let distance = node.position.distance(from: bubblesField.position)
            let nextDistance = nextNode.position.distance(from: bubblesField.position)
